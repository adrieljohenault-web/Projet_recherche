...
# même travail pour les sondes adcp : 
d_sonde_adcp = [[0. for _ in range(n_sortie)] for _ in range(3)]

for k in range(n_sortie):
    for i in range(3):
        # 1. CORRIGÉ : Utilisation de ADCPs[i] au lieu de sondes[i]
        d_sonde_adcp[i][k] = distance_euclidienne([bathy[k][0], bathy[k][1]], ADCPs[i])

# 2. CORRIGÉ : Uniformisation du nom en "adcp" partout (fini les inversions acdp/adcp)
points_inter_adcp = [[[] for _ in range(4)] for _ in range(3)]

for i in range(3):
    for j in range(4):
        closest_index_adcp = np.argmin(d_sonde_adcp[i])
        closest_distance_adcp = np.min(d_sonde_adcp[i])
        points_inter_adcp[i][j] = [closest_index_adcp, closest_distance_adcp]
        d_sonde_adcp[i][closest_index_adcp] = 10**10

# Somme des distances des 4 points pour chaque sonde adcp
...
# 3. CORRIGÉ : Remise en ordre des indices j et i pour correspondre à la structure des sondes
